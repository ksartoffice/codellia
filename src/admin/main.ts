...
const HTML_WORD_WRAP_STORAGE_KEY = 'cazeart.html.wordWrap';
...
async function main() {
  const cfg = window.CAZEART;
  const postId = cfg.post_id;
  const mount = document.getElementById('cazeart-app');
  if (!mount) return;
  ...
  if (cfg.setupRequired) {
    if (!cfg.setupRestUrl || !wp?.apiFetch) {
      ui.app.textContent = __( 'Setup wizard unavailable.', 'cazeart-live-code-editor');
      return;
    }
    ...
    try {
      ...
      console.error('[CazeArt] Setup failed', error);
      ui.app.textContent = __( 'Setup failed.', 'cazeart-live-code-editor');
      return;
    } finally {
      setupHost.remove();
    }
  ...
  const registerSaveShortcut = (
    ...
    editorInstance.addAction({
      id: 'cazeart.save',
      label: __( 'Save', 'cazeart-live-code-editor'),
  ...
  const registerHtmlWordWrapAction = (
    ...
    editorInstance.addAction({
      id: 'cazeart.toggleHtmlWordWrap',
      label: __( 'Toggle HTML word wrap', 'cazeart-live-code-editor'),
  ...
  const buildPreviewTemplateModeUrl = (url: string, templateModeValue: string) => {
    ...
    try {
      ...
      if (templateModeValue && templateModeValue !== 'default') {
        previewUrl.searchParams.set('cazeart_template_mode', templateModeValue);
      } else {
        previewUrl.searchParams.delete('cazeart_template_mode');
      }
  ...
  const buildPreviewRefreshUrl = (url: string) => {
    ...
    try {
      ...
      refreshUrl.searchParams.set('cazeart_refresh', Date.now().toString());
      return refreshUrl.toString();
    } catch {
      const hasQuery = url.includes('?');
      const hashIndex = url.indexOf('#');
      const suffix = `${hasQuery ? '&' : '?'}cazeart_refresh=${Date.now()}`;
      if (hashIndex === -1) {
        return url + suffix;
      }
    ...
    },
    {
      ...
      onUpdatePostIdentity: async ({ title, slug }) => {
        if (!cfg.settingsRestUrl || !wp?.apiFetch) {
          return { ok: false, error: __( 'Settings unavailable.', 'cazeart-live-code-editor') };
        }
        try {
          ...
          if (!response?.ok) {
            return { ok: false, error: response?.error || __( 'Update failed.', 'cazeart-live-code-editor') };
          }
          ...
          return {
            ok: false,
            error: error?.message || __( 'Update failed.', 'cazeart-live-code-editor'),
          };
        }
      },
  ...
  createSnackbar('info', __( 'Loading Monaco...', 'cazeart-live-code-editor'), NOTICE_IDS.monaco);
  ...
  const handleBeforeUnload = (event: BeforeUnloadEvent) => {
    ...
    event.returnValue = __( 'You may have unsaved changes.', 'cazeart-live-code-editor');
  };
  ...
  const openMediaModal = () => {
    if (typeof wp?.media !== 'function') {
      createSnackbar(
        'error',
        __( 'Media library is unavailable.', 'cazeart-live-code-editor'),
        NOTICE_IDS.media,
        NOTICE_ERROR_DURATION_MS
      );
      return;
    ...
    const frame = wp.media({
      frame: 'post',
      state: 'insert',
      title: __( 'Select media to insert into HTML.', 'cazeart-live-code-editor'),
      button: {
        text: __( 'Insert into HTML', 'cazeart-live-code-editor'),
      },
      multiple: false,
    });
    ...
    frame.on('insert', (selectionArg: any) => {
      ...
      if (!html) {
        createSnackbar(
          'warning',
          __( 'The selected media has no URL and was not inserted.', 'cazeart-live-code-editor'),
          NOTICE_IDS.media,
          NOTICE_ERROR_DURATION_MS
        );
        return;
  ...
  const normalizeAttributes = (attrs: { name: string; value: string }[]) => {
    ...
    for (let i = attrs.length - 1; i >= 0; i -= 1) {
      ...
      if (!name || name === 'data-cazeart-id' || !isValidAttributeName(name) || seen.has(name)) {
        continue;
      }
      seen.add(name);
