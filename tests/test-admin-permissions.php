...
class Codellia_Admin_Redirect_Exception extends Exception {
...
class Test_Admin_Permissions extends WP_UnitTestCase {
	...
	public function test_action_create_new_post_creates_draft_and_redirects_to_editor(): void {
		...
		$before_ids   = $this->get_codellia_post_ids();
		$original_get = $_GET;
		$_GET         = array(
			'post_type' => Post_Type::POST_TYPE,
			'_wpnonce'  => wp_create_nonce( Admin::NEW_POST_NONCE_ACTION ),
		...
		$_GET = $original_get;
		$after_ids = $this->get_codellia_post_ids();
	...
	private function get_codellia_post_ids(): array {
		return get_posts(
			array(
				'post_type'              => Post_Type::POST_TYPE,
				'post_status'            => 'any',
	...
	private function capture_redirect( callable $callback ): string {
		$redirect_filter = function ( $location ) {
			throw new Codellia_Admin_Redirect_Exception( (string) $location );
		};
		...
		} catch ( Codellia_Admin_Redirect_Exception $e ) {
			return $e->location;
		} finally {
