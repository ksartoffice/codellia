<?php
/**
 * Front-end single-page/public visibility tests for CazeArt.
 *
 * @package CazeArt
 */
...
class CazeArt_Frontend_Redirect_Exception extends Exception {
...
class CazeArt_Frontend_Template_Exception extends Exception {
}
...
class Test_Frontend_Single_Page_Controls extends WP_UnitTestCase {
	...
	public function test_maybe_redirect_single_page_redirects_when_disabled(): void {
		...
		update_post_meta( $post_id, '_cazeart_single_page_enabled', '0' );
		$original_wp_query = $this->set_query_for_post( $post_id, $post );

		$redirect_target = home_url( '/cazeart-landing/' );
		$redirect_filter = static function () use ( $redirect_target ) {
			return $redirect_target;
		};
		add_filter( 'cazeart_single_page_redirect', $redirect_filter );

		$wp_redirect_filter = static function ( $location ) {
			throw new CazeArt_Frontend_Redirect_Exception( (string) $location );
		};
		...
		} catch ( CazeArt_Frontend_Redirect_Exception $e ) {
			$this->assertSame( $redirect_target, $e->location );
		} finally {
			remove_filter( 'wp_redirect', $wp_redirect_filter );
			remove_filter( 'cazeart_single_page_redirect', $redirect_filter );
	...
	public function test_maybe_redirect_single_page_sets_404_when_filter_requests_it(): void {
		...
		$template_path = trailingslashit( get_temp_dir() ) . 'cazeart-test-404-template.php';
		file_put_contents( $template_path, '<?php throw new CazeArt_Frontend_Template_Exception("404-template-loaded");' );
		...
		} catch ( CazeArt_Frontend_Template_Exception $e ) {
	...
	public function test_exclude_single_page_from_search_query_adds_meta_filter(): void {
		...
		$this->assertSame( '_cazeart_single_page_enabled', $meta_query[0][0]['key'] ?? '' );
		$this->assertSame( 'NOT EXISTS', $meta_query[0][0]['compare'] ?? '' );
		$this->assertSame( '_cazeart_single_page_enabled', $meta_query[0][1]['key'] ?? '' );
		$this->assertSame( '1', $meta_query[0][1]['value'] ?? '' );
	}

	public function test_maybe_disable_autop_removes_filters_for_cazeart_posts(): void {
		$post_id = $this->create_cazeart_post();
		$post    = get_post( $post_id );
		$this->assertInstanceOf( WP_Post::class, $post );
	...
	public function test_maybe_override_template_uses_layout_templates_and_preview_override(): void {
		...
		update_post_meta( $post_id, '_cazeart_template_mode', 'frame' );
		$frame_template = Frontend::maybe_override_template( 'theme-template.php' );
		$this->assertStringContainsString( 'templates/single-cazeart-frame.php', str_replace( '\\', '/', $frame_template ) );

		update_post_meta( $post_id, '_cazeart_template_mode', 'theme' );
		$theme_template = Frontend::maybe_override_template( 'theme-template.php' );
		$this->assertSame( 'theme-template.php', $theme_template );

		update_post_meta( $post_id, '_cazeart_template_mode', 'default' );
		$GLOBALS['wp_query']->set( 'cazeart_preview', '1' );
		$GLOBALS['wp_query']->set( 'cazeart_template_mode', 'standalone' );
		$preview_template = Frontend::maybe_override_template( 'theme-template.php' );
		$this->assertStringContainsString( 'templates/single-cazeart-standalone.php', str_replace( '\\', '/', $preview_template ) );

		$this->restore_query( $original_wp_query );
	}

	private function create_cazeart_post(): int {
