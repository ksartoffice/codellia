<?php
/**
 * Front-end single-page/public visibility tests for Codellia.
 *
 * @package Codellia
 */
...
class Codellia_Frontend_Redirect_Exception extends Exception {
...
class Codellia_Frontend_Template_Exception extends Exception {
}
...
class Test_Frontend_Single_Page_Controls extends WP_UnitTestCase {
	...
	public function test_maybe_redirect_single_page_redirects_when_disabled(): void {
		...
		update_post_meta( $post_id, '_codellia_single_page_enabled', '0' );
		$original_wp_query = $this->set_query_for_post( $post_id, $post );

		$redirect_target = home_url( '/codellia-landing/' );
		$redirect_filter = static function () use ( $redirect_target ) {
			return $redirect_target;
		};
		add_filter( 'codellia_single_page_redirect', $redirect_filter );

		$wp_redirect_filter = static function ( $location ) {
			throw new Codellia_Frontend_Redirect_Exception( (string) $location );
		};
		...
		} catch ( Codellia_Frontend_Redirect_Exception $e ) {
			$this->assertSame( $redirect_target, $e->location );
		} finally {
			remove_filter( 'wp_redirect', $wp_redirect_filter );
			remove_filter( 'codellia_single_page_redirect', $redirect_filter );
	...
	public function test_maybe_redirect_single_page_sets_404_when_filter_requests_it(): void {
		...
		$template_path = trailingslashit( get_temp_dir() ) . 'codellia-test-404-template.php';
		file_put_contents( $template_path, '<?php throw new Codellia_Frontend_Template_Exception("404-template-loaded");' );
		...
		} catch ( Codellia_Frontend_Template_Exception $e ) {
	...
	public function test_exclude_single_page_from_search_query_adds_meta_filter(): void {
		...
		$this->assertSame( '_codellia_single_page_enabled', $meta_query[0][0]['key'] ?? '' );
		$this->assertSame( 'NOT EXISTS', $meta_query[0][0]['compare'] ?? '' );
		$this->assertSame( '_codellia_single_page_enabled', $meta_query[0][1]['key'] ?? '' );
		$this->assertSame( '1', $meta_query[0][1]['value'] ?? '' );
	}

	public function test_maybe_disable_autop_removes_filters_for_codellia_posts(): void {
		$post_id = $this->create_codellia_post();
		$post    = get_post( $post_id );
		$this->assertInstanceOf( WP_Post::class, $post );
	...
	private function create_codellia_post(): int {
