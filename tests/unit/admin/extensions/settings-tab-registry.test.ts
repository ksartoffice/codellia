...
const clearGlobals = () => {
  delete (window as any).CAZEART_EXTENSION_API;
  delete (window as any).CAZEART_SETTINGS_TAB_QUEUE;
};
...
describe('settings tab registry', () => {
  ...
  it('flushes queued tabs when the API initializes', async () => {
    const mount = vi.fn();
    (window as any).CAZEART_SETTINGS_TAB_QUEUE = [
      {
        id: 'ai',
        label: 'AI Editing',
        order: 20,
    ...
    expect(Array.isArray((window as any).CAZEART_SETTINGS_TAB_QUEUE)).toBe(true);
    expect((window as any).CAZEART_SETTINGS_TAB_QUEUE).toHaveLength(0);
  ...
  it('registers and unregisters tabs at runtime', async () => {
    const registry = await loadRegistry();
    const api = (window as any).CAZEART_EXTENSION_API;
